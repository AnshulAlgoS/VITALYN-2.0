...
export function AppHeader() {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem("vitalynUser");
    if (stored) {
      try {
  ...
  const displayName = user?.displayName || "Dr. Sarah Chen";
  const userRole = user?.role === "doctor" ? "Medical Professional" : user?.role === "patient" ? "Patient" : "Chief Resident";
