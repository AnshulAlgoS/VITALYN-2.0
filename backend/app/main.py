...
def login(payload: LoginRequest):
    ...
    # For demo, we still check password but allow any email to be passed back
    if payload.password != expected["password"]:
...
def send_otp(payload: SendOtpRequest):
    # Normalize phone number: remove spaces and non-digit characters except +
    phone = "".join(c for c in payload.phone_number if c.isdigit() or c == "+")
    role_key = payload.role.lower()
    ...
    if twilio_client:
        try:
            # For WhatsApp, the "to" number must also be prefixed with "whatsapp:"
            to_whatsapp = f"whatsapp:{phone}" if not phone.startswith("whatsapp:") else phone
            
            message = twilio_client.messages.create(
                body=f"Your Vitalyn verification code is: {otp}",
    ...
    return {
        ...
        "sid": message.sid if twilio_client else None,
    }
...
def verify_otp(payload: VerifyOtpRequest):
    ...
    if stored["otp"] != payload.otp:
        stored["attempts"] += 1
        return {"success": False, "error": f"Invalid OTP. {3 - stored['attempts']} attempts remaining"}
